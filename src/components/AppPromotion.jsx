...
// Same "coming soon" mobile-app promo as the original site. The email
// capture is currently visual-only (no backend endpoint requested) — happy
// to wire it to Notification/lead storage if you want it functional.
    ...
    <section className="overflow-hidden bg-emerald-900 text-paper py-16">
      <div className="mx-auto max-w-7xl px-4 grid items-center gap-14 lg:grid-cols-2">
        <motion.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.55 }} className="flex flex-col gap-6">
          <span className="w-fit rounded-full bg-white/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-emerald-200">Coming Soon</span>
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <span className="flex items-center gap-2 rounded-xl border border-white/15 px-4 py-2.5 text-xs font-medium text-white/80"><Play size={15} /> Google Play</span>
            <span className="flex items-center gap-2 rounded-xl border border-white/15 px-4 py-2.5 text-xs font-medium text-white/80"><Apple size={15} /> App Store</span>
          </div>
        </motion.div>
